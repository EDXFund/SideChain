	...
	if n != nil {
		...
		for uint64(fp.nodeCnt)+head.Number-n.number > maxNodeCount && fp.root != nil {
			...
			for i, nn := range fp.root.children {
				if rawdb.ReadCanonicalHash(f.pm.chainDb, f.chain.ShardId(),nn.number) == nn.hash {
...
func (f *lightFetcher) peerHasBlock(p *peer, hash common.Hash, number uint64) bool {
	...
	return rawdb.ReadCanonicalHash(f.pm.chainDb, f.chain.ShardId(), fp.root.number) == fp.root.hash && rawdb.ReadCanonicalHash(f.pm.chainDb, f.chain.ShardId(), number) == hash
}
